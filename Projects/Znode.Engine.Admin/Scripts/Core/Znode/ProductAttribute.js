...
var ProductAttribute = /** @class */ (function (_super) {
    ...
    ProductAttribute.prototype.AddNewRow = function (jsonArray) {
        ...
        html += $("#IsCategory").val() === "False" ? '<td class="is-default"><div id="isdefault' + _defaultValueCount + '"><input id="IsDefault_' + _defaultValueCount + '" name="IsDefault" type="radio" value="False" class="dirty"><span class="lbl padding-8" data-test-selector="IsDefaultchecked_' + _defaultValueCount + '"></span></div></td>' : "";
    ...
    ProductAttribute.prototype.ValidateDefaultValue = function (indexid, defaultvalueCode, swatchText, mediaId) {
        ...
        if (textboxes.length == emptytextboxes.length) {
            ...
            $("#defaultCodeVal-" + indexid).text('').text(ZnodeBase.prototype.getResourceByKeyName("OnlyAlphanumericareAllowed")).addClass("field-validation-error").show();
            $('#DefaultValueCode-' + indexid).addClass('input-validation-error');
            return false;
        }
...
$(document).on(dyanmic_grid_loaded, function () {
    DynamicGrid.prototype.FilterButtonPress = function (control) {
        ...
        $('.filtervalueinput').children().each(function (index, value) {
            var element = $(value);
            if (element.attr('name') == 'Total' || element.attr('name') == 'TotalReturnAmount') {
