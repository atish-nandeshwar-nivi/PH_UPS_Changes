...
var Checkout = /** @class */ (function (_super) {
    ...
    Checkout.prototype.SubmitOrderCheckout = function () {
                ...
                if ($("#dynamic-allowesterritories").length > 0) {
                    ...
                    switch (paymentType.toLowerCase()) {
                        case "credit_card":
                            ...
                            break;
                        case "cod":
                            $("#btnCompleteCheckout,#btnCompleteCheckout2").prop("disabled", false);
                            $("#btnCompleteCheckout,#btnCompleteCheckout2").show();
        ...
        if (!(paymentType.toLowerCase() == "ach")) {
            $("#btnCompleteCheckout,#btnCompleteCheckout2").prop("disabled", true);
        }
    };
    ;
    ...
    Checkout.prototype.ShowHidePayPalButton = function () {
        ...
        if (paymentOption == "paypal_express")
            $("#btnCompleteCheckout,#btnCompleteCheckout2").hide();
    ...
    Checkout.prototype.CalculateShipping = function (ShippingclassName, isCalculateCart) {
        ...
        if (form.length > 0) {
            ...
            if (typeof $("#hdnShipperTypes").val() === "undefined") {
                return;
            }
            var shipOptNames = $("#hdnShipperTypes").val().split(",");
            if ($("#hdnAllFreeShipping").val() == "1") {
                shipOptNames[0] = "Free_Shipping";
                $("#h_shipping_option").prop("checked", true);
            }
            var shippingOptionId = $("input[name='" + shipOptNames[0] + "']:checked").val();
            $("#h_shipping_option").val(shippingOptionId);
            var shippingOption2Id = 0;
            if (shipOptNames.length > 1) {
                shippingOption2Id = $("input[name='" + shipOptNames[1] + "']:checked").val();
            }
            var shippingOption3Id = 0; //Atish ups
            if (shipOptNames.length > 1) {
                shippingOption3Id = $("input[name='" + shipOptNames[2] + "']:checked").val();
            }
            var shippingAddressId = $("#shipping-content").find("#AddressId").val();
            var shippingCode = $("input[name='" + shipOptNames[0] + "']:checked").attr("data-shippingCode");
            var additionalInstruction = $('#AdditionalInstruction').val();
            var isQuoteRequest = $('#IsQuoteRequest').val();
            var jobName = $("#JobName").val();
            var isPendingOrderRequest = $('#IsPendingOrderRequest').val();
            var shippingsclass = $("input[name='" + shipOptNames[0] + "']").first().data('shippingsclass');
            $("#hndShippingclassName").val(shippingsclass);
            $("#messageBoxContainerId").hide();
            //if (shippingsclass.toLowerCase() == (Constant.ZnodeCustomerShipping).toLowerCase()) {
            //    $("#customerShippingDiv").show();
            //}
            //else { 
            //    $("#customerShippingDiv").hide();
            //}
            if (shippingOptionId == null || shippingOptionId == "" || typeof shippingOptionId === "undefined") {
                return false;
            }
            if (shipOptNames.length > 1 && (shippingOption2Id == null || shippingOption2Id == 0)) {
                return false;
            }
            else if (shipOptNames.length > 2 && (shippingOption3Id == null || shippingOption3Id == 0)) //Atish ups
             {
                return false;
            }
            else {
            ...
            if (allowSubmit) {
                $(".btnCompleteCheckout,#btnCompleteCheckout2").attr("disabled", false);
                if (form.attr('action').match("shippingOptionId")) {
                    var url = form.attr('action').split('?')[0];
                    form.attr('action', "");
                    form.attr('action', url);
                ...
                form.attr('action', form.attr('action') + "?shippingOptionId=" + shippingOptionId + "&shippingOption2Id="
                    + shippingOption2Id + "&shippingOption3Id="
                    + shippingOption3Id + "&shippingAddressId=" + shippingAddressId + "&shippingCode=" + shippingCode
                    + "&additionalInstruction=" + additionalInstruction + "" + "&isQuoteRequest=" + isQuoteRequest + "&isCalculateCart="
                    + isCalculateCart + "&isPendingOrderRequest=" + isPendingOrderRequest + "&jobName=" + jobName
                    + "&omsSavedCardLineItemIds=" + omsSavedCardLineItemIds + "&optionalfeesIds=" + optionalfeesIds); //Atish ups above
                form.submit();
            }
        }
        $("#btnCompleteCheckout,#btnCompleteCheckout2").prop("disabled", false);
    };
    Checkout.prototype.grayoutPaymentOption = function (grayout) {
        if (grayout) {
            $('input[name=PaymentOptions], #paymentBlock button').attr("disabled", true);
            $("#paymentBlock").addClass("grayout");
        }
        else {
            $('input[name=PaymentOptions], #paymentBlock button').attr("disabled", false);
            $("#paymentBlock").removeClass("grayout");
        }
    };
    Checkout.prototype.isShippingSelected = function () {
        if (typeof ($("#hdnShipperTypes").val()) === "undefined") {
            return false;
        }
        var shipOptNames = $("#hdnShipperTypes").val().split(",");
        var shippingOptionId = $("input[name='" + shipOptNames[0] + "']:checked").val();
        if (!(shippingOptionId > 0) && $("#FreeShipping").attr("checked")) {
            shippingOptionId = $("#FreeShipping").val();
        }
        var shippingOption2Id = 0;
        if (shipOptNames.length > 1) {
            shippingOption2Id = $("input[name='" + shipOptNames[1] + "']:checked").val();
            if (!(shippingOption2Id > 0) && $("#FreeShipping").attr("checked")) {
                shippingOption2Id = $("#FreeShipping").val();
            }
        }
    ...
    Checkout.prototype.ShowPaymentOptions = function (data) {
        ...
        if (paymentId != null && paymentId != "" && paymentId != "undefined") {
            ...
            $('#divCardconnect #iframebody,#divpurchase-order').hide();
        ...
        if ($("#hdnGatwayName").val() != undefined && $("#hdnGatwayName").val().length > 0) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if ($("#hdnAnonymousUser").val() == "true" || gatewayCode.toLowerCase() == "payflow" || !IsAnonymousUser) {
                        $("#Save-credit-card").hide();
                    }
                    else {
    ...
    Checkout.prototype.LogAuthNetIframeResponse = function (response) {
        Endpoint.prototype.LogAuthNetIframeResponse(parseInt($("#hdnPortalId").val()), response, function () { });
    };
    ...
    Checkout.prototype.AmazonShippingOptions = function (OrderReferenceId, paymentSettingId, total, accessToken) {
        $("#loaderId").html(" <div class='loader-inner' style='margin:0 auto;text-align:center;padding:20px;'><img src= '../Content/Images/loading.svg' alt= 'Loading' class='dashboard-loader' /></div>");
        Endpoint.prototype.AmazonShippingOptions(OrderReferenceId, paymentSettingId, total, accessToken, function (response) {
            $(".shipping-method").html(response);
            $("#loaderId").html("");
        });
    };
    ...
    Checkout.prototype.IsCheckoutDataValid = function () {
        ...
        var shippingOptionValue = $("input[name='" + shipOptNames[0] + "']:checked").val();
        //Nivi code end
        //var shippingOptionValue = $("input[name='ShippingOptions']:checked").val();
        var isBillingAddresOptional = $("#IsBillingAddressOptional").val();
        $("#errorAccountNumber").hide();
        ...
        else if (shipOptNames.length == 2 && $("input[name='" + shipOptNames[1] + "']:checked").val() === undefined) {
            ZnodeNotification.prototype.DisplayNotificationMessagesHelper("Select second shipping option", "error", isFadeOut, fadeOutTime);
            isValid = false;
            Checkout.prototype.HideLoader();
        }
        else if ($("#hndShippingclassName").val() != undefined && $("#hndShippingclassName").val() == Constant.ZnodeCustomerShipping && ($("#AccountNumber").val() == undefined || $("#AccountNumber").val() == "") && ($("#ShippingMethod").val() == undefined || $("#ShippingMethod").val() == "")) {
    ...
    //Kalviyo Changes(Register page signup and Cookied while login) Start
    Checkout.prototype.KlaviyoSubscribe = function (listId) {
        var userType = ZnodeBase.prototype.GetParameterValues("mode");
        ...
        if ($("#UserName").val() != null || userType == "guest") {
            var settings = {
                "async": true,
                "crossDomain": true,
    ...
    Checkout.prototype.GetCookie = function (cname) {
        var name = cname + "=";
    ...
    Checkout.prototype.BindOrderSummaryForPendingOrder = function (data) {
        if (data.Response.IsOABRequired) {
            $("#summary-details").html(data.html);
...
$(document).on('blur', "input#CredidCardCVCNumberSaved", function () {
    Checkout.prototype.ValidateCVV();
});
