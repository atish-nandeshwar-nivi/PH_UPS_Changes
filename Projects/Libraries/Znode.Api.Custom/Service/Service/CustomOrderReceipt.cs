...
namespace Znode.Api.Custom.Service.Service
{
    public class CustomOrderReceipt : ZnodeOrderReceipt
    {
        ...
        public CustomOrderReceipt()
        {
        }

        public CustomOrderReceipt(ZnodeOrderFulfillment order)
        {
            Order = order;
        }

        public CustomOrderReceipt(OrderModel orderOrderModel)
        {
            OrderModel = orderOrderModel;
        }

        public CustomOrderReceipt(ZnodeOrderFulfillment order, ZnodeShoppingCart shoppingCart)
        {
            Order = order;
            ShoppingCart = shoppingCart;
        }

        public CustomOrderReceipt(ZnodeOrderFulfillment order, string feedbackUrl)
        {
            Order = order;
            FeedbackUrl = feedbackUrl;
        }

        public CustomOrderReceipt(string cultureCode)
        {
        ...
        public override string GenerateHtmlResendReceiptWithParser(string receiptHtml, string templateCode = "")
        {
        ...
        public override void BuildOrderLineItem(DataTable multipleAddressTable, DataTable orderLineItemTable, DataTable multipleTaxAddressTable, OrderModel Order, DataTable returnedOrderlineItemTable, string templateCode = "")
        {
        ...
        public override DataRow SetOrderLineItemTable(DataTable orderLineItemTable, OrderLineItemModel lineitem, decimal extendedPrice, string templateCode = "")
        {
        ...
        public override DataTable SetOrderAmountData(OrderModel Order, string templateCode = "")
        {
        ...
        public override DataTable SetOrderData(OrderModel Order, string templateCode = "")
        {
            ...
            if (!string.IsNullOrEmpty(Order.Custom5))
            {
                var OptionalFeeID = Order.Custom5.Split(',');
                Order.Custom5 = string.Empty;
                string conn = "";
                foreach (var FeeID in OptionalFeeID)
                {
                    Order.Custom5 += conn += OptionalFee[FeeID];
                    conn = ", ";
        ...
        public override string CreateOrderReceipt(string template, string templateCode = "")
        {
        ...
        public override void BuildReturnedOrderAmountTable(string title, decimal amount, DataTable orderAmountTable, string cultureCode = "")
        {
            //gets +/- signs based on the order summary field label.
            string sign = GetSignBasedOnSummaryTitle(title);
        ...
        // Builds the order line item table.
        public override void BuildOrderLineItem(DataTable multipleAddressTable, DataTable orderLineItemTable, DataTable multipleTaxAddressTable, string templateCode = "")
        {
            List<OrderLineItemModel> OrderLineItemList = Order?.OrderLineItems.GroupBy(p => new { p.OmsOrderShipmentId }).Select(g => g.First()).ToList();
            IEnumerable<OrderShipmentModel> orderShipments = OrderLineItemList.Select(s => s.ZnodeOmsOrderShipment);
                ...
                foreach (OrderLineItemModel lineitem in Order.OrderLineItems.Where(x => x.OmsOrderShipmentId == orderShipment.OmsOrderShipmentId))
                {
                    ...
                    if (lineitem.OrderLineItemCollection?.Any(x => x.OrderLineItemRelationshipTypeId != Convert.ToInt32(ZnodeCartItemRelationshipTypeEnum.Bundles)) ?? false)
                    {
                        foreach (OrderLineItemModel childLineItem in lineitem.OrderLineItemCollection)
                        {
                            ...
                            if (orderLineItemTable != null)
                            {
                                ...
                                {
                                    if (IsJson(shoppingCartItem.Custom3))
                                    {
                                        var uomModeldata = Newtonsoft.Json.JsonConvert.DeserializeObject<PHUomModel>(shoppingCartItem.Custom3);
                                        orderlineItemDbRow["Price"] = GetFormatPriceWithCurrency(shoppingCartItem.UnitPrice, uomModeldata.PriceUnitDescription);//For price unit description instead of UOM
                                    }
                                    else
                                    {
                                        orderlineItemDbRow["Price"] = GetFormatPriceWithCurrency(shoppingCartItem.UnitPrice, shoppingCartItem.Custom3);
                                    }
                    ...
                    else
                    {
                        ...
                        if (orderLineItemTable != null)
                        {
                            ...
                            {
                                orderlineItemDbRow["Price"] = GetFormatPriceWithCurrency(shoppingCartItem.UnitPrice, shoppingCartItem.Custom3);//For price unit description instead of UOM
                            }
        ...
        public override string GetPersonaliseAttributesDetail(List<PersonaliseValueModel> PersonaliseValuesDetail)
        {
            string personaliseAttributeHtml = string.Empty;
        ...
        public override string GetFormatPriceWithCurrency(decimal priceValue, string uom = "", string cultureCode = "")
        {
        ...
        public override string GetOrderReceiptHtml(string templatePath, string templateCode = "")
        ...
        public override string GetOrderResendReceiptHtml(string templatePath, string templateCode = "")
        => GenerateHtmlResendReceiptWithParser(templatePath, templateCode);

        //Generates the HTML used in email receipts.
        public override string GenerateOrderReceipt(string templateContent, string templateCode = "")
        {
        ...
        public override void SetOrderTrackingNumber(OrderModel order, DataRow orderRow) => orderRow["TrackingNumber"] = SetTrackingURL(order);
        ...
        //Get shipping type name based on provided shipping id
        public override string GetShippingType(int shippingId)
        {
        ...
        public override string CreateLowInventoryNotification(string template)
        {
            if (string.IsNullOrEmpty(template))
                return template;
        ...
        //gets +/- signs based on the order summary field label.
        protected override string GetSignBasedOnSummaryTitle(string title)
        ...
        // Builds the order amount table.
        protected override void BuildTaxSummaryOrderTable(DataTable orderAmountTable)
        {
            var row = orderAmountTable.NewRow();
        ...
        //to create order amount table
        protected override DataTable CreateSummaryTaxTable()
        {
            DataTable orderAmountTable = new DataTable();
        ...
        //to create order amount table
        protected override DataTable SetTaxSummaryHeadData()
        {
            DataTable orderAmountTable = new DataTable();
            orderAmountTable.Columns.Add("TaxName");
            orderAmountTable.Columns.Add("Rates");
        ...
        // Builds the order amount table.
        protected override void BuildTaxAmountTable(DataTable orderTaxAmountTable)
        {
        ...
        //to set order amount data
        protected override DataTable SetTaxSummaryData()
        {
            // Create order amount table
        ...
        protected override void BindSummaryLineDetails(TaxSummaryModel taxSummary, DataTable orderTaxAmountTable)
        {
        ...
        protected override void BindTaxSummaryDetails(DataTable orderAmountTable)
        {
            if (IsNotNull(Order?.TaxSummaryList) && Order?.TaxSummaryList?.Count() > 0)
            {
                foreach (TaxSummaryModel taxSummary in Order.TaxSummaryList)
        ...
        protected override bool IsTaxSummaryAvailable()
        {
